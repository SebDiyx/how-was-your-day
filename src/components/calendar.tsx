...
export function Calendar({ onDayClick, dayEntries }: CalendarProps) {
  ...
  const getDayEntry = (day: Date): DayEntry | undefined => {
    return dayEntries.find((entry) => format(entry.date, "yyyy-MM-dd") === format(day, "yyyy-MM-dd"))
  }
    ...
    <div className="w-full bg-gradient-to-b from-amber-50 to-white p-6 rounded-2xl border border-amber-200 shadow-inner">
      <div className="flex items-center justify-between mb-6">
        <h2 className="text-2xl font-bold text-amber-800 flex items-center">
          <span className="bg-yellow-300 w-8 h-8 rounded-full mr-2 flex items-center justify-center border-2 border-green-500">
            🌻
          </span>
      ...
      <div className="grid grid-cols-7 gap-1 text-center font-medium mb-2">
        {["Sun", "Mon", "Tue", "Wed", "Thu", "Fri", "Sat"].map((day, index) => (
          <div key={day} className="text-amber-800 bg-amber-100 py-2 rounded-lg text-sm">
            {day}
          </div>
        ))}
      </div>
      <div className="mt-2 grid grid-cols-7 gap-2">
        {days.map((day, dayIdx) => {
          ...
          return (
            <div key={day.toString()} className={cn("aspect-square", dayIdx === 0 && `col-start-${getDay(day) + 1}`)}>
              <button
                type="button"
                ...
                className={cn(
                  ...
                  isToday(day) && !dayEntry && "border-green-500 font-bold bg-green-50",
